...
int bound (Node *,int);
void branch(Node*,int);
int getOptimalCost(Node *);
int * assignment(int len, int** kosten)
{
	Node * root = new Node(-1, -1, len, nullptr, kosten);
	branch(root, len);
	cout << "Optimal Cost : " << getOptimalCost(root)<<endl;
...
void showAssignment(int * array, int len)
{
	for (int i = 0; i < len; i++)
	{
		cout << "Person " << i+1 << " gets " << "Job " << array[i] + 1 << endl;
	}
}
int getOptimalCost(Node * node)
{
	int cost = 0;
	for (int i = 0; i < node->len; i++)
	{
		cost += node->table[i][node->rootpath[i]];
	}
	return cost;
}
#endif //BRANCH_H